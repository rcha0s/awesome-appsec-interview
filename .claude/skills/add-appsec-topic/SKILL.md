...
DIAGRAMS:
  ...
  - ARCHITECTURE-FIRST RULE: if the topic has multiple named static components (an API server, an agent, a broker, a control plane vs data plane split, several node types) AND a dynamic flow/sequence diagram, give the components diagram FIRST, as a plain \`flowchart\` showing the components and their static relationships (no numbered steps, no attack framing), then the sequence/flow diagram second, showing the same components in motion. A reader needs the map before the route. Do not merge both into one diagram; two smaller diagrams beat one dense one, and the split also means each individually stays legible. Example: docs/85-kubernetes.md should read this way once corrected (see Anti-patterns below for the current gap).
  - When you do include a sequence/flow diagram: protocol docs get a full-handshake sequence diagram with the attack surface annotated (Note lines or highlighted nodes); attack docs get an attack-chain flowchart end-to-end. Ground every label in claims already in the draft, do not introduce new facts via the diagram.
  ...
  - LAYOUT GOTCHA (verified against this site's rendering): 3+ independent, unconnected top-level \`subgraph\` blocks in one flowchart lay out side by side in a single wide row, shrinking every node's text to near-illegibility once there are more than two. Never leave 3+ sibling subgraphs unconstrained. Either split them into separate small diagrams (preferred when each variant is genuinely independent, e.g. "four deployment variants"), or force a grid with invisible rank-only links between pairs (\`A ~~~ B\` creates no visible edge but pins A and B to the same rank, so \`A ~~~ B\` then \`C ~~~ D\` then \`B ~~~ C\` produces a 2x2 grid instead of a 1x4 row). Check any diagram with 3+ subgraphs against this before finishing.
...
CONCISENESS:
  - Long, dense paragraphs get skimmed past by engineers, not read. This is doubly true for architecture-heavy or spec-heavy topics (zero trust, federation, protocol deep dives) where the temptation is to narrate every detail in prose. Prefer several short paragraphs over one long one; prefer a diagram or a table over a paragraph that is really just describing a static structure in words. If a paragraph exceeds roughly 5-6 sentences, look for a natural split or a structure (table, diagram, short list) that says the same thing faster.
  - This does not relax the citation-honesty, invariant-naming, or rubric-coverage requirements above. Say the same substance in less space, do not cut substance to hit a length target.
