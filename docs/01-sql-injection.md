...
### Parameterized queries vs stored procedures vs ORMs: when to use which

All three enforce the same invariant when used correctly (user data never reaches the parser as SQL text), so the choice is not about injection prevention per se, it is about where the query lives, who owns it, and what secondary properties you get.
