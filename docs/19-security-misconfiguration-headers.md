...
4. Set the full response-header suite, mapping each to the attack it blunts.
   ...
   - `Set-Cookie` attributes: `HttpOnly` (script cannot read the cookie, blunting XSS cookie theft), `Secure` (HTTPS-only, no cleartext leak), `SameSite=Lax`/`Strict` (CSRF reduction; `None` requires `Secure`). Prefer the `__Host-` prefix: the browser only accepts a `__Host-` cookie if it is `Secure`, has no `Domain` attribute (host-locked, not shared to subdomains), and `Path=/`. `__Secure-` requires the `Secure` flag. These prefixes are enforced by the browser, so a subdomain or a network attacker on `http://` cannot overwrite them (cookie tossing/fixation).
