...
- **No `__Host-` prefix**: without it, a subdomain or a network attacker can inject or override the cookie, since a broader `Domain` scope means any host under it can set a same-named cookie the browser cannot distinguish from the legitimate one (cookie tossing, attack technique 8 below). In a multi-tenant deployment where tenants live on sibling subdomains, this is cross-tenant cookie poisoning: a compromised or malicious tenant plants a cookie that rides along to a different tenant's origin. `__Host-` requires `Secure`, forbids `Domain`, and forces `Path=/`, binding the cookie to the exact host over HTTPS; it is OWASP's recommendation for session IDs<sup>[[1]](#ref1)</sup>. `__Secure-` is the weaker variant when subdomain sharing is required.
