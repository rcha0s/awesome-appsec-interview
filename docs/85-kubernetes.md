...
Kyverno and Gatekeeper install as ValidatingWebhookConfigurations. Each webhook has a `failurePolicy` that controls what the API server does when the webhook is unreachable: `Ignore` admits the request (fail-open), `Fail` rejects it (fail-closed). A common misconfiguration is `failurePolicy: Ignore` on a policy webhook, sometimes chosen because the operator worried about the webhook taking the cluster down. An attacker with `delete pods` in the policy namespace deletes the policy pod, or DoSes the webhook service, then submits the previously-blocked manifest during the outage. If any subject can `update validatingwebhookconfigurations`, they can flip `failurePolicy` themselves or narrow the `namespaceSelector` to exclude their target namespace<sup>[[8]](#ref8)</sup>.
