...
**(a) Device agent + gateway.** Most granular telemetry, requires managed endpoints.
...
**(b) Enclave gateway.** What most enterprises use for legacy on-prem apps; weakens the per-resource property since one gateway fronts many resources.
...
**(c) Resource portal.** The BeyondCorp-popularized pattern, agentless but loses endpoint visibility.
...
**(d) Device application sandboxing.** Niche: trust is scoped to one sandboxed app on the device, not the whole endpoint.
...
Principal: The proxy replacement is the tactical win and often the first deliverable, but calling it zero trust is the mistake. The seven tenets in §2.1 are the definition, and tenets 3, 4, 6, and 7 are what a proxy alone does not give you. A proxy covers tenet 2 (encrypted regardless of network) and maybe tenet 6 (dynamic auth at the gate); if the decision is static and never fed by posture, threat intel, or session risk, that is a modern VPN, not zero trust. Capital One 2019 is the reference case: network-flat-plus-role-based-auth was called modern security, and the SSRF that reached instance metadata succeeded because network reachability was authorization, exactly what zero trust removes.
...
Principal: 800-207 (2020) assumed the caller is a user with a device. 800-207A (2023) reflects that in cloud-native the caller is often a workload, and that VPCs and subnets are administrative units, not trust boundaries. It introduces the two-tier policy model (identity-tier primary, network-tier defense in depth), names service mesh, API gateway, and sidecar proxy as PEP patterns, and mandates workload identity as a first-class PDP input. Cite it whenever "same VPC" is offered as a reachability rationale. Incident grounding: cross-tenant breaches in multi-tenant Kubernetes, where a compromised pod reached another tenant's namespace over a flat internal network, are the class 800-207A is written against.
...
Principal: The variant choice has real operational consequences. Criteria-based is auditable but accumulates exceptions until the ruleset itself becomes the vulnerability. Score-based looks sophisticated but fails two ways: weights drift toward accepting the most common legitimate pattern until users are trained to power through step-up, and one high-weight signal (usually a device-compliance boolean) tends to dominate the score alone. Contextual beats singular but only if the signal store resists low-and-slow poisoning. There is no default right answer, only which failure mode is more expensive in the environment. Incident grounding: SolarWinds is the abstract form of signal poisoning, a single high-weight signal ("this binary is signed by the vendor") dominated downstream trust with no independent verification.
...
Principal: MFA is one input to the trust algorithm, not the algorithm, and this is the most common category error in the interview. An eight-hour token issued after MFA that the PEP never re-checks against a revocation channel violates the per-session and dynamic-enforcement tenets regardless of the MFA. If the trust algorithm treats "MFA present" as "high assurance" while the method is push-approve, the input classification is wrong and every downstream decision inherits the error; phishing-resistant MFA (WebAuthn) is a specific CISA Advanced-maturity requirement, not a synonym for MFA. Ask which tenet is enforced by mechanism versus claimed by policy. Incident grounding: the 2022 Uber and Okta breaches both started with push-fatigue MFA bombing, where "we have MFA" collapsed because the method itself was not phishing-resistant.
...
Principal: The tenet requires revocation, and most deployments have not implemented it, which is where a large share of real incidents land. For OAuth-shaped tokens the access token is a locally-validated bearer credential, so revocation needs either short lifetimes (bounded blast radius, not true revocation) or a PA-to-PEP callback channel (true revocation). OIDC id tokens are typically not revocable at all, which is why logout often does not reach downstream apps relying on the id token alone. mTLS-fronted service-to-service uses CRL or OCSP-stapling instead. Ask which channel is wired and what happens when it's down; "the session continues" means the tenet is nominal. Incident grounding: the 2024 Snowflake credential-stuffing incidents show valid credentials without device-binding or continuous posture check letting sessions persist even after passwords were rotated elsewhere.
...
Principal: The tradeoff is posture granularity versus access breadth. Portal gives you the browser's view: user agent, some fingerprinting, TLS characteristics, whatever JavaScript can attest to. Agent gives you firmware, patch state, EDR verdict, disk encryption, running processes, at the cost of managed-endpoint requirements and support burden. Portal suits HTTP-shaped resources and forces tunneling for arbitrary TCP. Most real deployments hybridize: agent for managed employee endpoints, portal for contractors and BYOD, enclave gateway reluctantly for legacy apps. Incident grounding: third-party-contractor compromise campaigns (the Target 2013 HVAC-vendor pattern) target contractors precisely because they ride the portal path with the weakest posture visibility, so scope and step-up have to compensate.
...
Principal: If an agent authenticates as the user, holding and replaying the user's access token, prompt injection has just phished the user's session; that is where credential passthrough and excessive agency become zero-trust failures. The zero-trust answer gives the agent its own workload identity, with user context carried as a delegated token via RFC 8693 exchange and RFC 8707 audience binding, so each downstream call is authorized against both the agent's identity and the scoped delegated context. 800-207A already treats workload identity as a first-class input; extending it to agents just means naming which identity the agent gets and how tightly the delegated context is scoped. Treating the agent as the user is impersonation with extra steps. Incident grounding: indirect prompt injection through document content causing an assistant to invoke tools at user privilege is the archetype; a properly delegated model requires the injected instruction to forge a new audience-bound token, which token exchange prevents.
...
Principal: These four are complementary and get cited together in real work. 800-207 is the definition. 800-207A applies it to cloud-native architecture. 1800-35 shows what a real deployment looks like across 19 vendor-stack reference implementations. CISA ZTMM v2.0 turns it into a program-management answer: a specific requirement per pillar and stage, not a philosophy. EO 14028 and OMB M-22-09 are the federal drivers; DoD ZTRA v2.0 is the DoD-specific variant with a different pillar taxonomy. Naming versions and years accurately signals the candidate actually read them. Incident grounding: SolarWinds, Colonial Pipeline, and the Microsoft Exchange on-prem chain are the incidents EO 14028 was written to prevent recurrence of, which locates the framework in an actual threat history rather than a compliance checklist.
