...
11. CRLF injection in the URL to smuggle raw commands over plain http://. Some HTTP clients do not sanitize newlines in the URL (older curl, PHP streams, historical Java, Python `urllib` before the CVE-2016-5699 fix), so a payload like `http://127.0.0.1:6379/%0d%0aCONFIG%20SET%20dir%20/var/spool/cron/%0d%0a%0d%0a` decodes the percent-encoded CR/LF and splices them into the request line the client writes to the socket. A line-based server (Redis, Memcached, SMTP) treats the injected lines as its own protocol commands and executes them.
...
- "gopher is blocked, can you still hit Redis?" Mid: try `dict://`. Principal: if the HTTP client is one of the historical CRLF-permissive parsers (older curl, PHP streams, pre-CVE-2016-5699 Python `urllib`), plain `http://target/%0d%0aCONFIG%20SET%20dir%20/var/spool/cron/%0d%0a...` splices raw newlines into the request line and drives the same Redis inline-protocol chain over `http://` with no exotic scheme. The fix is a client that rejects CR/LF in the request target, plus a validator that rejects control characters after full decoding.
- "Your product exports user-submitted content to PDF, walk me through the SSRF surface." Mid: strip external image URLs. Principal: the renderer's DOM is the sink. `<iframe>`, `<object>`, `<link rel=stylesheet>`, `<img>`, and SVG `<image xlink:href>` all trigger server-side fetches whose responses are baked into the PDF or PNG, so you get a reflected read on a headless pipeline. Fix at the markup layer (strip external subresource tags), the renderer flag layer (disable `file://` and local file access), and the network layer (renderer segment cannot reach metadata or internal admin planes). Application URL filtering on the submitted document is not sufficient because the attacker controls the entire DOM the renderer loads.
- "Your IP pin defeats DNS rebinding, but does it hold under keep-alive and redirects?" Mid: it pins per connect. Principal: pinning one `connect()` is necessary but not sufficient. If the outgoing Host header still names the attacker's hostname, virtual-hosted internal backends and reverse proxies may route by Host and reach an unintended target; if a keep-alive pool reuses the socket for a second logical request, the validation layer's assumed second `getaddrinfo` never fires; on redirect the new hop needs a fresh resolve-validate-pin cycle rather than any reused state. The invariant is one resolve, one is_global validation, one connect, one rewritten Host header, per hop.
...
- Python CVE-2016-5699 (CRLF injection in `urllib` HTTP request URLs): https://nvd.nist.gov/vuln/detail/CVE-2016-5699
- wkhtmltopdf usage and security options (`--disable-local-file-access` and restricted-access flags): https://wkhtmltopdf.org/usage/wkhtmltopdf.txt
